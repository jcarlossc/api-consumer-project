...
class ApiClientStrategy(Protocol):
    ...
    def fetch(
        ...
        """
            Executa uma operação de busca (fetch) em uma API, retornando um objeto
            `ResponseModel` contendo os dados estruturados da resposta.

            Parameters
            ----------
            endpoint : str, optional
                Caminho ou recurso específico da API a ser consultado. Caso não seja
                fornecido, a implementação pode assumir um endpoint padrão.
            params : dict[str, Any], optional
                Parâmetros adicionais enviados na requisição, como filtros, paginação
                ou autenticação.
