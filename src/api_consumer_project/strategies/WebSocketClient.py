...
class WebSocketClient(ApiClientStrategy):
    ...
    def __init__(self, url: str) -> None:
        """
            ...
            Args:
                url (str):
                    Endpoint WebSocket que será utilizado para enviar e receber mensagens.
                    Pode ser ``ws://`` ou ``wss://``.
        """
        self.url = url
    ...
    async def _async_fetch(self, message: str) -> ResponseModel:
        """
            Executa a operação assíncrona de envio e recebimento de dados no WebSocket.

            Args:
                message (str):
                    Mensagem a ser enviada ao servidor WebSocket.
                    Exemplo: "Hello WebSocket!".
                    Caso vazio, o servidor retornará a resposta padrão (se suportado).
